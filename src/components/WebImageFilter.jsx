...
export default function WebImageFilter({ config, hideControls=false, minimal=false, forceAfter=false, previewImageURL=null, onProcessed=null, onConfigApplied=null, initialView=null, compact=false, mirrored=false }) {
  ...
  const [warmthR, setWarmthR] = useState(0);
  const [warmthB, setWarmthB] = useState(0);
  const [warmthA, setWarmthA] = useState(0);
  const [mode, setMode] = useState('paint');
  const [view, setView] = useState(minimal || forceAfter ? 'after' : 'split');
  ...
  useEffect(() => {
    if (config && typeof config === 'object') {
      ...
      if (typeof config.sigma === 'number') setSigma(config.sigma)
      ...
      if (typeof config.warmthR === 'number') setWarmthR(config.warmthR)
      if (typeof config.warmthB === 'number') setWarmthB(config.warmthB)
      if (typeof config.warmthA === 'number') setWarmthA(config.warmthA)
      if (typeof config.filterType === 'string') setFilterType(config.filterType)
      if (initialView) { setView(initialView) }
      else if (!minimal && !forceAfter) setView('split')
      try { if (typeof onConfigApplied === 'function') onConfigApplied({
        intensity: (typeof config.intensity === 'number' ? config.intensity : intensity),
        sigma: (typeof config.sigma === 'number' ? config.sigma : sigma),
        brightness: (typeof config.brightness === 'number' ? config.brightness : brightness),
        contrast: (typeof config.contrast === 'number' ? config.contrast : contrast),
        warmthR: (typeof config.warmthR === 'number' ? config.warmthR : warmthR),
        warmthB: (typeof config.warmthB === 'number' ? config.warmthB : warmthB),
        warmthA: (typeof config.warmthA === 'number' ? config.warmthA : warmthA),
        filterType: (typeof config.filterType === 'string' ? config.filterType : filterType)
      }) } catch(_){ }
    }
  }, [config, initialView])
  ...
  useEffect(() => {
    ...
    try {
      ...
      img.onerror = function() { setProcessing(false); };
  ...
  function capturePhoto() {
    ...
    canvas.toBlob(function(blob) {
      if (!blob) { setError('Error al capturar la foto'); return; }
      setProcessing(true);
    ...
    try {
      ...
      [glCanvasRef.current, maskCanvasRef.current, outCanvasRef.current].forEach(function(c){
        if (c) {
          ...
          if (compact && c === outCanvasRef.current) { c.style.width = 'auto'; c.style.height = '100%'; }
          else { c.style.width = '100%'; c.style.height = compact ? '100%' : 'auto'; }
      ...
      setError('');
      if (!minimal && !forceAfter) setView('split');
      renderGL();
      setTimeout(function() { detectAndMaskFace(); }, 500);
    } catch (err) {
      console.error(err);
      setError(String(err.message || err));
    }
  ...
  useEffect(function() {
    if (image && faceMeshRef.current && !detectingFace) {
      const defaults = FILTERS[filterType] && FILTERS[filterType].defaults;
      const deep = FILTERS[filterType] && FILTERS[filterType].deep;
      if (defaults && !config) {
        if (typeof defaults.intensity === 'number') setIntensity(defaults.intensity);
        if (typeof defaults.sigma === 'number') setSigma(defaults.sigma);
        if (typeof defaults.brightness === 'number') setBrightness(defaults.brightness);
      }
      if (deep && !config) {
        const c = deep.contrast && typeof deep.contrast.value === 'number' ? deep.contrast.value : 1.0;
        const wr = deep.warmth && typeof deep.warmth.rPlus === 'number' ? deep.warmth.rPlus : 0;
        const wb = deep.warmth && typeof deep.warmth.bMinus === 'number' ? deep.warmth.bMinus : 0;
        const wa = deep.warmth && typeof deep.warmth.alpha === 'number' ? deep.warmth.alpha : 0;
        setContrast(c);
        setWarmthR(wr);
        setWarmthB(wb);
        setWarmthA(wa);
  ...
  async function detectFaceWithMesh(imageElement) {
    return new Promise((resolve, reject) => {
      if (!faceMeshRef.current) { reject(new Error('FaceMesh no inicializado')); return; }
      const tempCanvas = document.createElement('canvas');
      tempCanvas.width = imageElement.width;
      tempCanvas.height = imageElement.height;
      const tempCtx = tempCanvas.getContext('2d');
      tempCtx.drawImage(imageElement, 0, 0);
      faceMeshRef.current.onResults((results) => { resolve(results); });
  ...
  function getCenterOfPoints(landmarks, indices) {
    let sumX = 0, sumY = 0;
    indices.forEach(idx => { sumX += landmarks[idx][0]; sumY += landmarks[idx][1]; });
    return [sumX / indices.length, sumY / indices.length];
  }

  function getBoundingBox(landmarks, indices) {
    let minX = Infinity, minY = Infinity; let maxX = -Infinity, maxY = -Infinity;
    indices.forEach(idx => { const [x, y] = landmarks[idx]; minX = Math.min(minX, x); minY = Math.min(minY, y); maxX = Math.max(maxX, x); maxY = Math.max(maxY, y); });
  ...
  function drawSmoothRegion(ctx, landmarks, indices, intensity = 0.8, blur = 1.5) {
    ...
    ctx.fillStyle = gradient; ctx.beginPath(); ctx.arc(0, 0, maxRadius, 0, Math.PI * 2); ctx.fill(); ctx.restore();
  }

  function excludeRegion(ctx, landmarks, indices) {
    ctx.globalCompositeOperation = 'destination-out';
    ctx.beginPath();
    indices.forEach((idx, i) => { const [x, y] = landmarks[idx]; if (i === 0) { ctx.moveTo(x, y); } else { ctx.lineTo(x, y); } });
    ctx.closePath(); ctx.fillStyle = 'rgba(255, 255, 255, 1)'; ctx.fill();
    ctx.globalCompositeOperation = 'source-over';
  }

  async function detectAndMaskFace(retryCount = 0) {
    if (!image) { setProcessing(false); return; }
    if (!faceMeshRef.current) {
      try {
        const m = maskCanvasRef.current; if (!m) { setProcessing(false); return; }
        const ctx = m.getContext('2d'); if (!ctx) { setProcessing(false); return; }
        ctx.clearRect(0, 0, m.width, m.height);
    ...
    try {
      ...
      if (!results.multiFaceLandmarks || results.multiFaceLandmarks.length === 0) {
        if (retryCount < 2) { setTimeout(() => { detectAndMaskFace(retryCount + 1); }, 500); return; }
        const m = maskCanvasRef.current; const ctx = m && m.getContext('2d');
        if (ctx) { ctx.clearRect(0,0,m.width,m.height); ctx.fillStyle = 'rgba(255,255,255,1)'; ctx.fillRect(0,0,m.width,m.height); }
        syncMaskToGPU(); renderGL();
        return;
      }
      const landmarks = results.multiFaceLandmarks[0];
      const m = maskCanvasRef.current; const ctx = m.getContext('2d');
      ctx.clearRect(0, 0, m.width, m.height);
      const canvasW = m.width; const canvasH = m.height;
      const imgW = image.naturalWidth || image.width; const imgH = image.naturalHeight || image.height;
      const scaledLandmarks = landmarks.map(function(point) { return [point.x * canvasW, point.y * canvasH]; });
      if (filterType === 'wrinkles') { applyWrinklesMaskWithFaceMesh(ctx, scaledLandmarks); }
      else if (filterType === 'firmness') { applyFirmnessMaskWithFaceMesh(ctx, scaledLandmarks); }
      else if (filterType === 'brightness') { applyBrightnessMaskWithFaceMesh(ctx, scaledLandmarks); }
      else if (filterType === 'spots') { applySpotsMaskWithFaceMesh(ctx, scaledLandmarks); }
      else if (filterType === 'acne') { applyAcneMaskWithFaceMesh(ctx, scaledLandmarks); }
      syncMaskToGPU(); if (!minimal && !forceAfter) setView('split'); renderGL(); setFaceDetected(true);
    } catch (err) {
  ...
  function applyWrinklesMaskWithFaceMesh(ctx, scaledLandmarks) {
    ctx.globalCompositeOperation = 'source-over';
    const foreheadIndices = FACEMESH_INDICES.forehead; drawSmoothRegion(ctx, scaledLandmarks, foreheadIndices, 0.85, 1.3);
    const glabellaIndices = [6, 197, 195, 5, 168, 8, 9]; drawSmoothRegion(ctx, scaledLandmarks, glabellaIndices, 0.75, 1.0);
    const rightEyeOuterCorner = [33, 133, 155, 154, 153, 145, 144, 163]; drawSmoothRegion(ctx, scaledLandmarks, rightEyeOuterCorner, 0.8, 1.4);
    const leftEyeOuterCorner = [263, 362, 382, 381, 380, 374, 373, 390]; drawSmoothRegion(ctx, scaledLandmarks, leftEyeOuterCorner, 0.8, 1.4);
    const rightUnderEye = [133, 155, 154, 153, 145, 144, 163, 7]; drawSmoothRegion(ctx, scaledLandmarks, rightUnderEye, 0.7, 1.2);
    const leftUnderEye = [362, 382, 381, 380, 374, 373, 390, 249]; drawSmoothRegion(ctx, scaledLandmarks, leftUnderEye, 0.7, 1.2);
    const rightNasolabial = FACEMESH_INDICES.rightNasolabial; drawSmoothRegion(ctx, scaledLandmarks, rightNasolabial, 0.75, 1.5);
    const leftNasolabial = FACEMESH_INDICES.leftNasolabial; drawSmoothRegion(ctx, scaledLandmarks, leftNasolabial, 0.75, 1.5);
    const upperMouthArea = [2, 98, 97, 99, 75, 60, 20, 242, 97, 98, 2, 327, 326, 328, 305, 290, 250, 462, 1, 10, 164, 393]; drawSmoothRegion(ctx, scaledLandmarks, upperMouthArea, 0.85, 1.4);
    const lowerMouthArea = [61, 146, 91, 181, 84, 17, 314, 405, 321, 375]; drawSmoothRegion(ctx, scaledLandmarks, lowerMouthArea, 0.85, 1.6);
    const chinArea = [152, 377, 400, 378, 379, 365, 397, 288, 361]; drawSmoothRegion(ctx, scaledLandmarks, chinArea, 0.65, 1.3);
  ...
  function applyBrightnessMaskWithFaceMesh(ctx, scaledLandmarks) {
    ...
    const radiusX = bbox.width * 0.6; const radiusY = bbox.height * 0.7; const maxRadius = Math.max(radiusX, radiusY);
    ctx.save(); ctx.translate(center[0], center[1]); ctx.scale(radiusX / maxRadius, radiusY / maxRadius);
  ...
  function applySpotsMaskWithFaceMesh(ctx, scaledLandmarks) {
    ...
    const center = getCenterOfPoints(scaledLandmarks, faceOvalIndices);
    const bbox = getBoundingBox(scaledLandmarks, faceOvalIndices);
    const radiusX = bbox.width * 0.72; const radiusY = bbox.height * 0.82; const maxRadius = Math.max(radiusX, radiusY);
    ctx.save(); ctx.translate(center[0], center[1]); ctx.scale(radiusX / maxRadius, radiusY / maxRadius);
    ...
    const upperLipArea = [2, 98, 97, 99, 75, 60, 20, 242, 327, 326, 328, 305, 290, 250, 462]; drawSmoothRegion(ctx, scaledLandmarks, upperLipArea, 0.9, 1.2);
    const rightCheekbone = [205, 206, 207, 187, 123, 116, 111]; const leftCheekbone = [425, 426, 427, 411, 352, 345, 340];
    drawSmoothRegion(ctx, scaledLandmarks, rightCheekbone, 0.92, 1.2); drawSmoothRegion(ctx, scaledLandmarks, leftCheekbone, 0.92, 1.2);
    excludeRegion(ctx, scaledLandmarks, FACEMESH_INDICES.rightEyeUpper.concat(FACEMESH_INDICES.rightEyeLower));
    excludeRegion(ctx, scaledLandmarks, FACEMESH_INDICES.leftEyeUpper.concat(FACEMESH_INDICES.leftEyeLower));
    excludeRegion(ctx, scaledLandmarks, FACEMESH_INDICES.rightEyebrow);
    excludeRegion(ctx, scaledLandmarks, FACEMESH_INDICES.leftEyebrow);
    const innerLips = [61, 185, 40, 39, 37, 0, 267, 269, 270, 409, 291, 146, 91, 181, 84, 17, 314, 405, 321, 375];
  ...
  function applyAcneMaskWithFaceMesh(ctx, scaledLandmarks) {
    ...
    const center = getCenterOfPoints(scaledLandmarks, faceOvalIndices);
    const bbox = getBoundingBox(scaledLandmarks, faceOvalIndices);
    const radiusX = bbox.width * 0.75; const radiusY = bbox.height * 0.85; const maxRadius = Math.max(radiusX, radiusY);
    ctx.save(); ctx.translate(center[0], center[1]); ctx.scale(radiusX / maxRadius, radiusY / maxRadius);
    ...
    const noseArea = [6, 197, 195, 5, 4, 1, 19, 94, 2]; drawSmoothRegion(ctx, scaledLandmarks, noseArea, 0.98, 1.3);
    const chinArea = [152, 377, 400, 378, 379, 365, 397, 288, 361, 323, 454, 93]; drawSmoothRegion(ctx, scaledLandmarks, chinArea, 0.98, 1.4);
    const glabellaArea = [8, 9, 168, 6, 197, 195, 5]; drawSmoothRegion(ctx, scaledLandmarks, glabellaArea, 0.95, 1.0);
    const noseSidesRight = [218, 126, 142, 36, 205, 206]; const noseSidesLeft = [438, 355, 371, 266, 425, 426];
    drawSmoothRegion(ctx, scaledLandmarks, noseSidesRight, 0.95, 1.2); drawSmoothRegion(ctx, scaledLandmarks, noseSidesLeft, 0.95, 1.2);
    excludeRegion(ctx, scaledLandmarks, FACEMESH_INDICES.rightEyeUpper.concat(FACEMESH_INDICES.rightEyeLower));
    excludeRegion(ctx, scaledLandmarks, FACEMESH_INDICES.leftEyeUpper.concat(FACEMESH_INDICES.leftEyeLower));
    excludeRegion(ctx, scaledLandmarks, FACEMESH_INDICES.rightEyebrow);
    excludeRegion(ctx, scaledLandmarks, FACEMESH_INDICES.leftEyebrow);
  ...
  function applyFirmnessMaskWithFaceMesh(ctx, scaledLandmarks) {
    drawSmoothRegion(ctx, scaledLandmarks, FACEMESH_INDICES.rightCheek, 0.95, 1.4);
    drawSmoothRegion(ctx, scaledLandmarks, FACEMESH_INDICES.leftCheek, 0.95, 1.4);
    const rightUpperLidExtended = FACEMESH_INDICES.rightEyeUpper.concat([70, 63, 105, 66, 107]); drawSmoothRegion(ctx, scaledLandmarks, rightUpperLidExtended, 0.9, 1.1);
    const leftUpperLidExtended = FACEMESH_INDICES.leftEyeUpper.concat([300, 293, 334, 296, 336]); drawSmoothRegion(ctx, scaledLandmarks, leftUpperLidExtended, 0.9, 1.1);
    ...
    const centerCheekboneRight = [205, 206, 207, 187, 123]; drawSmoothRegion(ctx, scaledLandmarks, centerCheekboneRight, 0.85, 1.0);
    const centerCheekboneLeft = [425, 426, 427, 411, 352]; drawSmoothRegion(ctx, scaledLandmarks, centerCheekboneLeft, 0.85, 1.0);
    const jawlineRight = [172, 136, 150, 149, 176, 148]; drawSmoothRegion(ctx, scaledLandmarks, jawlineRight, 0.75, 1.2);
    const jawlineLeft = [397, 365, 379, 378, 400, 377]; drawSmoothRegion(ctx, scaledLandmarks, jawlineLeft, 0.75, 1.2);
    excludeRegion(ctx, scaledLandmarks, FACEMESH_INDICES.rightEyeUpper.concat(FACEMESH_INDICES.rightEyeLower));
    excludeRegion(ctx, scaledLandmarks, FACEMESH_INDICES.leftEyeUpper.concat(FACEMESH_INDICES.leftEyeLower));
  }
  ...
  useEffect(() => {
    ...
    function getCoordsFromEvent(e) {
      ...
      const x = (cx - rect.left) * scaleX; const y = (cy - rect.top) * scaleY; return { x: x, y: y };
    }
    function paintAt(x, y) {
      const r = brushSize / 2;
      const grad = ctx.createRadialGradient(x, y, r * Math.max(0, Math.min(1, hardness)), x, y, r);
      grad.addColorStop(0, 'rgba(255,255,255,1)'); grad.addColorStop(1, 'rgba(255,255,255,0)');
      ctx.globalAlpha = 1; ctx.globalCompositeOperation = (mode === 'paint') ? 'source-over' : 'destination-out';
      ctx.fillStyle = grad; ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.fill();
    }
    function onPointerDown(e) { e.preventDefault(); painting = true; try { canvas.setPointerCapture && canvas.setPointerCapture(e.pointerId); } catch(_){ } const p = getCoordsFromEvent(e); paintAt(p.x, p.y); syncMaskToGPU(); renderGL(); }
    function onPointerMove(e) { if (!painting) return; e.preventDefault(); const p = getCoordsFromEvent(e); paintAt(p.x, p.y); syncMaskToGPU(); renderGL(); }
  ...
  function renderGL() {
    try {
      const gl = glRef.current; if (!gl || !image) {
        const oc = outCanvasRef.current; if (oc && image) { const octx = oc.getContext('2d'); const w = imgSize.w, h = imgSize.h; if (w && h) { octx.clearRect(0,0,w,h); octx.drawImage(image, 0, 0, w, h); } }
        return;
      }
      const w = imgSize.w, h = imgSize.h; if (!w || !h) return;
      ...
      const u_contrast = gl.getUniformLocation(progComposeRef.current, 'u_contrast');
      const u_warmthR = gl.getUniformLocation(progComposeRef.current, 'u_warmthR');
      const u_warmthB = gl.getUniformLocation(progComposeRef.current, 'u_warmthB');
      const u_warmthAlpha = gl.getUniformLocation(progComposeRef.current, 'u_warmthAlpha');
      const u_filterType = gl.getUniformLocation(progComposeRef.current, 'u_filterType');
      gl.activeTexture(gl.TEXTURE0); gl.bindTexture(gl.TEXTURE_2D, texOrigRef.current); gl.uniform1i(u_orig, 0);
      gl.activeTexture(gl.TEXTURE1); gl.bindTexture(gl.TEXTURE_2D, texPongRef.current); gl.uniform1i(u_blur, 1);
      gl.activeTexture(gl.TEXTURE2); gl.bindTexture(gl.TEXTURE_2D, texMaskRef.current); gl.uniform1i(u_mask, 2);
      gl.uniform1f(u_int, intensity);
      gl.uniform1f(u_bright, brightness);
      gl.uniform1f(u_contrast, contrast);
      gl.uniform1f(u_warmthR, warmthR);
      gl.uniform1f(u_warmthB, warmthB);
      gl.uniform1f(u_warmthAlpha, warmthA);
      let filterId = 0; if (filterType === 'brightness') filterId = 1; else if (filterType === 'spots') filterId = 2; else if (filterType === 'acne') filterId = 3; else if (filterType === 'firmness') filterId = 4; gl.uniform1i(u_filterType, filterId);
      try { if (typeof onConfigApplied === 'function') onConfigApplied({ intensity, sigma, brightness, contrast, warmthR, warmthB, warmthA, filterType }) } catch(_){ }
      gl.bindFramebuffer(gl.FRAMEBUFFER, null);
      setViewport(gl, w, h);
      gl.clearColor(0,0,0,0);
      gl.clear(gl.COLOR_BUFFER_BIT);
      ...
      const octx = outCanvasRef.current && outCanvasRef.current.getContext('2d'); if (!octx) return;
      gl.finish();
      octx.clearRect(0,0,w,h);
      if (view === 'before') {
        octx.drawImage(image, 0, 0, w, h);
        if (filterType === 'spots') {
          try {
            ...
            const d = imgData.data; for (let i = 0; i < d.length; i += 4) { const gray = 0.299 * d[i] + 0.587 * d[i + 1] + 0.114 * d[i + 2]; d[i] = d[i + 1] = d[i + 2] = gray; }
            octx.putImageData(imgData, 0, 0);
          } catch (err) { console.error('Error aplicando escala de grises en "antes":', err); }
        }
      } else if (view === 'after') {
        const hasGL = !!srcCanvas && srcCanvas.width && srcCanvas.height;
        if (hasGL) { octx.drawImage(srcCanvas, 0, 0, w, h); } else { octx.drawImage(image, 0, 0, w, h); }
      } else if (view === 'mask') {
        octx.fillStyle = '#000000'; octx.fillRect(0, 0, w, h); octx.drawImage(maskCanvasRef.current, 0, 0, w, h);
      } else {
          ...
          if (splitPx > 0) {
            ...
            if (filterType === 'spots') {
              try {
                ...
                const d = imgData.data; for (let i = 0; i < d.length; i += 4) { const gray = 0.299 * d[i] + 0.587 * d[i + 1] + 0.114 * d[i + 2]; d[i] = d[i + 1] = d[i + 2] = gray; }
                octx.putImageData(imgData, 0, 0);
              } catch (err) { console.error('Error aplicando B&N en split izquierda:', err); }
            }
          }
          const rightW = w - splitPx; if (rightW > 0) octx.drawImage(srcCanvas, splitPx, 0, rightW, h, splitPx, 0, rightW, h);
        } else {
          // Mirrored: izquierda procesado, derecha original
          if (splitPx > 0) {
            octx.drawImage(srcCanvas, 0, 0, splitPx, h, 0, 0, splitPx, h);
          }
          const rightW = w - splitPx; if (rightW > 0) {
            octx.drawImage(image, splitPx, 0, rightW, h, splitPx, 0, rightW, h);
            if (filterType === 'spots') {
              try {
                const imgData = octx.getImageData(splitPx, 0, rightW, h);
                const d = imgData.data; for (let i = 0; i < d.length; i += 4) { const gray = 0.299 * d[i] + 0.587 * d[i + 1] + 0.114 * d[i + 2]; d[i] = d[i + 1] = d[i + 2] = gray; }
                octx.putImageData(imgData, splitPx, 0);
              } catch (err) { console.error('Error aplicando B&N en split derecha (mirrored):', err); }
            }
          }
        }
        octx.save();
        ...
        octx.lineWidth = 2;
        octx.beginPath();
        octx.moveTo(Math.floor(splitPx), 0);
        octx.lineTo(Math.floor(splitPx), h);
        octx.stroke();
        octx.fillStyle = 'rgba(0,0,0,0.15)';
        octx.beginPath();
        octx.arc(Math.floor(splitPx)+2, h/2+2, 14, 0, Math.PI*2);
        octx.fill();
        octx.fillStyle = '#ffffff';
        octx.beginPath();
        octx.arc(Math.floor(splitPx), h/2, 14, 0, Math.PI*2);
        octx.fill();
      ...
      try { if (typeof onProcessed === 'function') { const url = outCanvasRef.current && outCanvasRef.current.toDataURL('image/jpeg', 0.92); if (url) onProcessed(url); } } catch(_){ }
    } catch (err) {
  ...
  useEffect(() => {
    const canvas = outCanvasRef.current; if (!canvas) return; let dragging = false;
    function getX(e) { const rect = canvas.getBoundingClientRect(); const x = e.clientX - rect.left; let p = x / rect.width; if (mirrored) p = 1 - p; return Math.max(0, Math.min(1, p)); }
    function onDown(e) { if (view !== 'split') return; dragging = true; try { canvas.setPointerCapture && canvas.setPointerCapture(e.pointerId); } catch(_){} setSplitPos(getX(e)); }
    function onMove(e) { if (!dragging || view !== 'split') return; setSplitPos(getX(e)); }
    function onUp(e) { dragging = false; try { canvas.releasePointerCapture && canvas.releasePointerCapture(e.pointerId); } catch(_){} }
  ...
  useEffect(function() { renderGL(); }, [intensity, sigma, brightness, contrast, warmthR, warmthB, warmthA, view, splitPos]);
    ...
    return (
      <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
        <div className="absolute w-0 h-0 overflow-hidden">
          ...
          <canvas ref={maskCanvasRef} className="block" />
        </div>
        <canvas ref={outCanvasRef} className="block h-full w-auto" style={{background:'#ffffff', transform: mirrored ? 'scaleX(-1)' : 'none', transformOrigin:'center'}} />
        {processing && (
          <div className="absolute inset-0 bg-white/70 flex items-center justify-center">
            <div className="w-10 h-10 rounded-full border-4 border-neutral-300 border-t-neutral-900 animate-spin" />
          </div>
        )}
      </div>
    )
  ...
  return (
    <div className={`${minimal ? 'h-full' : 'min-h-screen'} w-full bg-neutral-100 text-neutral-900`}>
      <div className="mx-auto max-w-6xl p-6">
        {!minimal && !compact && (
          <header className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <input type="file" accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp" onChange={onFile} className="hidden" id="fileInput" />
              <label htmlFor="fileInput" className="rounded-xl border px-3 py-2 text-sm bg-white shadow hover:shadow-md cursor-pointer">Subir foto</label>
              <button onClick={startCamera} className="rounded-xl border px-3 py-2 text-sm bg-white shadow hover:shadow-md" disabled={showCamera}>Tomar foto</button>
        ...
        {minimal && !compact && (
          ...
          <input type="file" accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp" onChange={onFile} className="hidden" id="fileInputMinimal" />
          <label htmlFor="fileInputMinimal" className="px-4 py-2 rounded-full bg-gray-900 text-white shadow hover:bg-gray-800 cursor-pointer">Subir imagen</label>
          <button onClick={startCamera} className="px-4 py-2 rounded-full bg-md-green text-white shadow disabled:opacity-50" disabled={showCamera}>Usar cámara</button>
          <button onClick={downloadPNG} className="px-4 py-2 rounded-full border border-gray-300 text-gray-900 bg-white shadow disabled:opacity-50" disabled={!image}>Descargar</button>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-75 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full overflow-hidden">
              <div className="p-4 border-b flex items-center justify-between">
                ...
                <button onClick={stopCamera} className="text-neutral-500 hover:text-neutral-900 text-2xl leading-none">×</button>
              </div>
              ...
              <div className="p-4 flex gap-3 justify-center">
                <button onClick={capturePhoto} className="rounded-xl bg-neutral-900 text-white px-6 py-3 text-sm font-medium shadow-lg hover:bg-neutral-800">Capturar foto</button>
                <button onClick={stopCamera} className="rounded-xl border border-neutral-300 px-6 py-3 text-sm font-medium hover:bg-neutral-50">Cancelar</button>
              </div>
              <div className="px-4 pb-4 text-xs text-neutral-500 text-center">Asegúrate de tener buena iluminación frontal y mantén tu rostro centrado</div>
            </div>
          </div>
        )}

        {!minimal && (
          <div onDragOver={(e)=>{ try { e.preventDefault(); if (e.dataTransfer) e.dataTransfer.dropEffect='copy'; } catch(_){} }} onDrop={(e)=>{ try { e.preventDefault(); e.stopPropagation(); const dt = e.dataTransfer; const file = dt && dt.files && dt.files.length > 0 ? dt.files[0] : null; if (file) { handleFile(file); } else { setError('No se detectó ningún archivo en el arrastre. Arrastra un JPG/PNG/WebP desde tu equipo.'); } } catch(err) { console.error('drop error', err); setError('Error al procesar el arrastre. Usa el botón "Elegir archivo" como alternativa.'); } }} className="mb-4 rounded-xl border-2 border-dashed border-neutral-300 bg-white p-4 text-sm text-neutral-600">
            Arrastra **un archivo de imagen** (JPG/PNG/WebP) desde tu ordenador o usa el selector de archivo de arriba.
          </div>
        )}

        {error ? (<div className="mb-4 rounded-xl bg-red-50 border border-red-200 text-red-800 p-3 text-sm">{error}</div>) : null}
          ...
          <section className="lg:col-span-1 bg-white rounded-2xl shadow p-4 flex flex-col gap-4">
            ...
            <div className="border-t pt-3">
              ...
              <div className="flex flex-col gap-2">
                <button onClick={function(){ setFilterType('wrinkles'); if(image) { setTimeout(function() { detectAndMaskFace(); }, 100); } }} className={'rounded-lg px-3 py-2 text-sm border ' + (filterType==='wrinkles'?'bg-neutral-900 text-white':'bg-white')}>Arrugas</button>
                <button onClick={function(){ setFilterType('brightness'); if(image) { setTimeout(function() { detectAndMaskFace(); }, 100); } }} className={'rounded-lg px-3 py-2 text-sm border ' + (filterType==='brightness'?'bg-neutral-900 text-white':'bg-white')}>Luminosidad</button>
                <button onClick={function(){ setFilterType('spots'); if(image) { setTimeout(function() { detectAndMaskFace(); }, 100); } }} className={'rounded-lg px-3 py-2 text-sm border ' + (filterType==='spots'?'bg-neutral-900 text-white':'bg-white')}>Manchas / Tono</button>
                <button onClick={function(){ setFilterType('acne'); if(image) { setTimeout(function() { detectAndMaskFace(); }, 100); } }} className={'rounded-lg px-3 py-2 text-sm border ' + (filterType==='acne'?'bg-neutral-900 text-white':'bg-white')}>Acné / Imperfecciones</button>
                <button onClick={function(){ setFilterType('firmness'); if(image) { setTimeout(function() { detectAndMaskFace(); }, 100); } }} className={'rounded-lg px-3 py-2 text-sm border ' + (filterType==='firmness'?'bg-neutral-900 text-white':'bg-white')}>Firmeza (lift)</button>
              </div>
            ...
            <label className={'text-sm'}>
              ...
              <input type="range" min={0} max={1} step={0.01} value={intensity} onChange={function(e){setIntensity(parseFloat(e.target.value));}} className="w-full" disabled={controlsLocked} />
            </label>
            <label className={'text-sm'}>
              Suavizado (sigma): {sigma}
              <input type="range" min={1} max={10} step={1} value={sigma} onChange={function(e){setSigma(parseInt(e.target.value));}} className="w-full" disabled={controlsLocked} />
            </label>
            <label className={'text-sm'}>
              Luminosidad: {brightness > 0 ? '+' : ''}{brightness.toFixed(2)}
              <input type="range" min={-0.3} max={0.3} step={0.01} value={brightness} onChange={function(e){setBrightness(parseFloat(e.target.value));}} className="w-full" disabled={controlsLocked} />
            </label>
            <div className="border-t pt-3">
              <h3 className="font-medium text-sm mb-2">Pincel de máscara</h3>
              <label className="text-sm">Tamaño: {brushSize}px<input type="range" min={10} max={200} step={1} value={brushSize} onChange={function(e){setBrushSize(parseInt(e.target.value));}} className="w-full"/></label>
              <label className="text-sm">Dureza: {Math.round(hardness*100)}%<input type="range" min={0} max={1} step={0.05} value={hardness} onChange={function(e){setHardness(parseFloat(e.target.value));}} className="w-full"/></label>
        ...
        {minimal ? (
                compact ? (
                  <div className="relative w-full h-full">
                    ...
                    <canvas ref={outCanvasRef} className="absolute inset-0 w-full h-full block" style={{background:'#ffffff'}} />
                    {processing && (
                      <div className="absolute inset-0 bg-white/70 flex items-center justify-center">
                        <div className="w-10 h-10 rounded-full border-4 border-neutral-300 border-t-neutral-900 animate-spin" />
                      </div>
                  ...
                  <div className="p-2">
                    <div className="absolute w-0 h-0 overflow-hidden">
                      <canvas ref={glCanvasRef} className="block" style={{background:'#fafafa'}} />
                      <canvas ref={maskCanvasRef} className="block" style={{background:'#ffffff'}} />
                    </div>
                      ...
                      {processing && (
                        <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center">
                          <div className="flex flex-col items-center gap-3">
                            <div className="w-10 h-10 rounded-full border-4 border-neutral-300 border-t-neutral-900 animate-spin" />
                            <div className="text-sm text-neutral-700">Aplicando filtro…</div>
                          </div>
                        </div>
                      )}
                    </div>
